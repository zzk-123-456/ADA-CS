...
class DomainNetDataset(torch.utils.data.Dataset):
	def __init__(self, name, domain, split, transforms):
		...
		self.domain = domain
		self.split = split
		self.file_path = os.path.join('/data/s2024104100/CLUE-main/data/DomainNet-cleaned/domainnet_h5/', '{}_{}.h5'.format(self.domain, self.split))
...
class ASDADataset:
	...
	def get_dsets(self, normalize=True, apply_transforms=True):
		if self.name == "mnist":
			...
			RGB2Gray = transforms.Lambda(rgb_to_gray)
			train_transforms = transforms.Compose([
								   RGB2Gray,

...
def run_active_adaptation(cfg, source_model, src_dset, num_classes, device):
    ...
    src_train_loader, _, _ = src_dset.get_loaders()
    tgt_train_loader, _, _ = tgt_dset.get_loaders()
    ...
    budget_allocator = BudgetAllocator(budget=budget, cfg=cfg, if_second=if_second)
    ...
    for epoch in tqdm_rat:
        ...
        if if_second:
            curr_budget, curr_budget_second, used_budget, used_budget_second = budget_allocator.get_budget(epoch)
            all_used_budget = used_budget + used_budget_second
            all_curr_budget = curr_budget + curr_budget_second
            tqdm_rat.set_description('# Target labels={} Allowed labels={}'.format(used_budget, curr_budget))
            tqdm_rat.set_description('# \nFor Second Step Target labels={} Allowed labels={}'.format(used_budget_second, curr_budget_second))
            tqdm_rat.set_description('# \nAll Target labels={} All Allowed labels={}'.format(all_used_budget, all_curr_budget))
            tqdm_rat.refresh()
        else:
            curr_budget, used_budget = budget_allocator.get_budget(epoch)
            tqdm_rat.set_description('# \nTarget labels={} Allowed labels={}'.format(used_budget, curr_budget))
            tqdm_rat.refresh()

        # Select instances via AL strategy
        if curr_budget > 0:
            sampling_strategy.calcu_shift(src_dset)
            logging.info('\nSelecting instances...')
            idxs = sampling_strategy.query(curr_budget, epoch)
            idxs_lb[idxs] = True
            # Whether ADA-CS is used or not
            if if_second:
                if curr_budget_second > 0:
                    logging.info('\nSelecting instances for second step...')
                    idxs_2 = sampling_strategy.cal_proportion(idxs, idxs_lb,
                                                              src_train_loader, tgt_train_loader,
                                                              n2=curr_budget_second)
                    idxs_lb[idxs_2] = True
...
def main():
    ...
    logger = "{}_{}_{}_{}".format(args.timestamp, cfg.DATASET.NAME, cfg.ADA.AL, cfg.ADA.DA, cfg.SECOND.IF) if not args.nolog else None
